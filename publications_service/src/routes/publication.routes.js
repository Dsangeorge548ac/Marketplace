...
const { validateSession, requireVerified } = require('../middleware/auth');
...
// Protected Routes (regular products)
router.post('/', validateSession, requireVerified, (req, res, next) => {
...
// ── Mineral Publication Routes ────────────────────────────────────────────────
router.post('/minerals', validateSession, requireVerified, (req, res, next) => {
    console.log("Mineral - Auth passed, starting upload...");
    uploadFields(req, res, (err) => {
        if (err) {
            console.error("Multer Error (minerals):", err);
            const statusCode = err.code === 'LIMIT_FILE_SIZE' ? 413 : 500;
            return res.status(statusCode).json({ error: true, message: "Upload failed: " + err.message });
        }
        next();
    });
}, pubController.createMineralPublication);

router.put('/minerals/:id', validateSession, (req, res, next) => {
    uploadFields(req, res, (err) => {
        if (err) {
            console.error("Multer Error (minerals update):", err);
            const statusCode = err.code === 'LIMIT_FILE_SIZE' ? 413 : 500;
            return res.status(statusCode).json({ error: true, message: "Upload failed: " + err.message });
        }
        next();
    });
}, pubController.updateMineralPublication);
