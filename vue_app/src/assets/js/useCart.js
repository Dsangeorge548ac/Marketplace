...
export function useCart() {
    ...
    function generatePDFPreview() {
        ...
        autoTable(doc, {
            ...
            styles: {
                ...
                cellPadding: 2,
                lineColor: [0, 0, 0],
                lineWidth: 0, // Set to 0 to remove inner cell borders
                valign: 'middle',
                textColor: [0, 0, 0]
            },
            columnStyles: {
        ...
        // Draw outer border for Customer Info Table
        doc.setLineWidth(0.1);
        doc.setDrawColor(0, 0, 0);
        doc.rect(margin, customerStartY, pageWidth - (margin * 2), doc.lastAutoTable.finalY - customerStartY);

        // --- Subheader Table ---
        const subheaderStartY = doc.lastAutoTable.finalY + 0;
        autoTable(doc, {
            startY: subheaderStartY,
            margin: { left: margin, right: margin },
            theme: 'plain',
            styles: {
                fontSize: 8,
            ...
            columnStyles: {
                0: { halign: 'left' },
                1: { halign: 'center', cellWidth: 55, fontStyle: 'none' },
                2: { halign: 'center', cellWidth: 45, fontStyle: 'none' },
                3: { halign: 'center', cellWidth: 45, fontStyle: 'none' }
            },
            ...
            didDrawCell: function (data) {
                if (data.section === 'head') {
                    ...
                    // Línea vertical izquierda (para dividir las columnas)
        ...
        // Draw outer border for Subheader Table
        doc.setLineWidth(0.1);
        doc.setDrawColor(0, 0, 0);
        doc.rect(margin, subheaderStartY, pageWidth - (margin * 2), doc.lastAutoTable.finalY - subheaderStartY);
        ...
        const mainItemsStartY = doc.lastAutoTable.finalY + 5;
        autoTable(doc, {
            startY: mainItemsStartY, // Attached to subheader
            margin: { left: margin, right: margin },
            theme: 'plain',
            styles: {
                fontSize: 8,
            ...
            columnStyles: {
                0: { halign: 'left' },
                1: { halign: 'center', cellWidth: 21, fontStyle: 'none' },
        ...
        // Draw outer border for Main Items Table
        doc.setLineWidth(0.1);
        doc.setDrawColor(0, 0, 0);
        doc.rect(margin, mainItemsStartY, pageWidth - (margin * 2), doc.lastAutoTable.finalY - mainItemsStartY);
        ...
        doc.setLineWidth(0.1);
        doc.setDrawColor(0, 0, 0);
        doc.rect(pageWidth - margin - summaryWidth, mainTableFinalY, summaryWidth, doc.lastAutoTable.finalY - mainTableFinalY);
    ...
    async function checkout(router) {
        ...
        try {
            ...
            if ((userData.status || '').toLowerCase() !== 'verificado') {
                Swal.fire({
                    icon: 'warning',
                    title: 'Cuenta no verificada',
                    text: 'Debes verificar tu cuenta antes de poder realizar un pedido.',
            ...
            if (orderResponse.id || orderResponse.message) {
                ...
                clearCart();
                if (router) router.push('/dashboard/orders');
            }
