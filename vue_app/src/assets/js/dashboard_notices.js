...
export function useDashboardNotices() {
    ...
    async function loadPage(page = 1) {
        ...
        try {
            ...
            const cacheKey = `cache_notices_${page}_${search.value}`;
            ...
            sessionStorage.setItem(cacheKey, JSON.stringify({ notices: notices.value, count: totalCount.value }));

        } catch (err) {
            console.error('Error cargando noticias:', err)
            notices.value = []
            totalCount.value = 0
            sessionStorage.removeItem(`cache_notices_${page}_${search.value}`);
        }
    }
