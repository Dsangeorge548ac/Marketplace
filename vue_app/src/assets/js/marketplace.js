...
export function useMarketplace(autoLoad = true) {
    ...
    function loadPage(page = 1, source = 'auto', customLimit = null) {
        if (loadTimeout) clearTimeout(loadTimeout);
        
        loadTimeout = setTimeout(async () => {
             await _executeLoadPage(page, source, customLimit);
        }, 300);
    }

    async function _executeLoadPage(page = 1, source = 'auto', customLimit = null) {
        ...
        try {
            ...
            if (products.value.length === 0) {
                ...
                Toast.fire({ icon: 'success', title: `Filtro aplicado: ${totalCount.value} resultado` });
            } else {
            ...
            if (axiosLib.isCancel(err)) return;
    ...
    onMounted(() => {
        if (!autoLoad) return;
        try {
            // Si regresamos al home, limpiamos los filtros silenciosamente para no afectar los sliders
            if (route.path === '/') {
                isClearing.value = true;
                categories.value = ['todas'];
                subCategories.value = [];
            ...
            if (route.query.search) {
                ...
                loadPage(1, headerAlertIsOpen ? 'search' : 'auto', route.path === '/' ? 50 : null);
            } else {
                loadPage(1, 'auto', route.path === '/' ? 50 : null);
            }
        } catch (e) {
